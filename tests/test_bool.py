...
def test_single_bool(value: bool, step: SequenceCallStep):
    ...
    result = string_measurement.add_result(result=value)
...
def test_multiple_bool_limits(
    names: list[str],
    values: list[bool],
    step: SequenceCallStep,
):
    numeric_measurement = step.add_test_step(
        "Multiple Bool Limit Step",
        MultipleBooleanLimitStep,
    )
    for name, value in zip(names, values, strict=True):
        result = numeric_measurement.add_result(name, result=value)
